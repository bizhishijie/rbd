function RcNew = sortByZigzagCell(Rc, threNear, threSide, threWall)
...
for ii = 1:length(idMin) %对zigzag遍历
    ...
    while ~ismember(idRcTemp, idMax) %只要没找到另外一端
        ...
        idRcTemp = idNearList(dsearchn(theta', sgn * pi / 3)); %最接近某个角度的
        RcNew{ii} =[RcNew{ii} Rc(:, idRcTemp)]; %将找到的小球坐标加到RcNew里
        sgn = -sgn; %转换向上找还是向下找
        fprintf('%d\t%d\n', ii, idRcTemp)
    end
