...
save_address=['D:\Research\short_wx2\1.1\'];%save adress which you should alter
wx=1.1;wy=5;N=15000;%wx wy and the Number of each deposition's paticles
xs=[];dxs=[];%x_save deltax_save
de42=[];dz42=[];dy42=[];de30=[];dz30=[];dy30=[];de56=[];dz56=[];dy56=[];%de for delta the number is pn
pll30=[];pll42=[];pll56=[];pln30=[];pln42=[];pln56=[];%pll for period last length, pln for period last number 
pl30m=[];pl30a=[];pl42m=[];pl42a=[];pl56m=[];pl56a=[];%period length(pl) m for mean and a for all, mean is a whole "big period"'s pl's mean
tl30=[];tl42=[];tl56=[];tn30=[];tn42=[];tn56=[];%transient length/number
...
%     [Rc,tole_degree_loop]=Sequential_deposition3D_(wx,wy,N);
    ...
    load([save_address file_name '.mat'],'Rc','tole_degree_loop','wx','wy')
    Rc_old=Rc;
%     Period_find_tranisent_length%use a script instead of a function 'cause it's too much for saving
    Period_random_deposition%same here
    ...
    ,'dy30t','de56t','dz56t','dy56t','pll30t','pll42t','pll56t','pln30t','pln42t','pln56t','pl30mt','pl30at'...
    ,'pl42mt','pl42at','pl56mt','pl56at','tl30t','tl42t','tl56t','tn30t','tn42t','tn56t')

    xs=[xs x_save];dxs=[dxs dx_save];%save all data
